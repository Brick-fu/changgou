...
#参数值由pom文件传递,完整包名changgou-eureka-0.0.1.tar
baseTarName="${package-name}" #压缩包名称  changgou-eureka-0.0.1
...
baseTarPath="${basePath}/${baseTarName}.tar"  #压缩包路径
...
{
    echo "解压---------------------------------------------"
    echo "压缩包路径：${baseTarPath}"
    if [ ! `find ${baseTarPath}` ]
    then
        echo "不存在压缩包：${baseTarPath}"
    else
        echo "解压文件磁盘路径：${baseDirPath}/${baseTarPath}"
        echo "开始解压..."

        #解压命令
#        unzip -od ${baseDirPath}/${baseTarPath} ${baseTarPath}
        tar -xf ${baseTarName}.tar
        #设置执行权限
        chmod +x ${baseDirPath}/${baseTarName}/${packageName}
...
#        cd ${baseDirPath}/${baseTarName}
        ...
        then
            ...
            nohup ${baseDirPath}/${packageName} >/dev/null 2>&1 &
        fi
